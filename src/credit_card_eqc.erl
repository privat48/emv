...
prop_valid_number() -> ?FORALL(N, valid_cc_number(), credit_card:is_valid(N)).
prop_visa_card() -> ?FORALL(N, visa_cc_number(), credit_card:type(N) =:= visa).
prop_amex_card() -> ?FORALL(N, amex_cc_number(),  credit_card:type(N) =:= amex).
prop_mastercard_card() -> ?FORALL(N, mastercard_cc_number(),  credit_card:type(N) =:= mastercard).
prop_discover_card() -> ?FORALL(N, discover_cc_number(),  credit_card:type(N) =:= discover).
